...
static void get_test_process_output(char *cmd, FILE *fp, char *output, char *test) {
...
static void single_element(char *cmd, FILE *fp, char *output) {
    ...
    fp = popen(cmd, "r");
    get_test_process_output(cmd, fp, output, "<sorted> single_element -");
    pclose(fp);
}
...
static void four_elements(char *cmd, FILE *fp, char *output) {
    ...
    fp = popen(cmd, "r");
    get_test_process_output(cmd, fp, output, "<sorted> four_elements --");
    pclose(fp);
}
...
static void nine_elements(char *cmd, FILE *fp, char *output) {
    sprintf(cmd, "./push_swap %d %d %d %d %d %d %d %d %d 2>&1", 0, 1, 2, 3, 4,
            5, 6, 7, 8);
    fp = popen(cmd, "r");
    get_test_process_output(cmd, fp, output, "<sorted> nine_elements --");
    pclose(fp);
}
